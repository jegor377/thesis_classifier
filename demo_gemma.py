...
import torch
import gradio as gr
from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig, RobertaTokenizer, RobertaModel
import torch
...
def generate_article(statement, model, tokenizer):
    ...
    return full_output.split("[/INST]")[-1].strip().replace("\t", "").replace("\n", "")